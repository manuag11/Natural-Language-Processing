...
/* This class can be used to train the Stanford Parser on Brown and test on wSJ. 
...
class StatsParsing1 {
   ...
   * on a corpus.
   *
   * Usage: {@code java StatsParsing1 [train_corpus] [train_size] [self-train_corpus] [self-train_size] [test_corpus] [test_size]}
   *
   */
  ...
  public static void main(String[] args) {
    ...
    selftraintreebank.loadPath(selftraintreebankPath);
    MemoryTreebank testtreebank = new MemoryTreebank("utf-8");
    testtreebank.loadPath(testtreebankPath);
    
    ArrayList<MemoryTreebank> list=new ArrayList<MemoryTreebank>();
    //Split the train corpus (Brown in our case) into 90-10.
    File folder = new File(traintreebankPath);
    File[] listOfFiles = folder.listFiles();
    for (File file : listOfFiles) {
        MemoryTreebank m=new MemoryTreebank("utf-8");
