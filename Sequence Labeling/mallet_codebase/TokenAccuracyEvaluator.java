...
public class TokenAccuracyEvaluator extends TransducerEvaluator
{
	...
	public void evaluateInstanceList (TransducerTrainer trainer, InstanceList instances, String description,HashMap<String,Boolean> map,boolean flag) 
  {
		int numCorrectTokens;
		int totalTokens;
                // numCorrectTokens_OOV is the number of correct tokens counting only OOV words
                int numCorrectTokens_OOV=0;
                // totalTokens_OOV is the total number of OOV tokens encountered
                int totalTokens_OOV=0;
		...
		for (int i = 0; i < instances.size(); i++) {
			...
			for (int j = 0; j < trueOutput.size(); j++) {
				totalTokens++;
                                //check if map does not contain the current word
                                if(!map.containsKey(input.get(j).toString()))
                                {
                                    totalTokens_OOV++;
                                    // check if the trueOutput matches the predictedOutput
                                    if (trueOutput.get(j).equals(predOutput.get(j)))
                                    {
                                        numCorrectTokens_OOV++;
                                    }
                                }
				if (trueOutput.get(j).equals(predOutput.get(j)))
                                {
		...
		logger.info (description +" accuracy="+acc);
                //display the OOV accuracy only if flag is true
                if(flag)
                {
                    //check if the total number of OOV tokens is zero
                    if(totalTokens_OOV!=0)
                    {
                        double acc_OOV=((double)numCorrectTokens_OOV)/totalTokens_OOV;
                        logger.info ("OOV accuracy="+acc_OOV);
                    }
                    else
                    {
                        logger.info ("OOV accuracy=undefined");
                    }
                }
                logger.info ("Percentage of OOV tokens ="+(double)totalTokens_OOV/totalTokens);
                }
