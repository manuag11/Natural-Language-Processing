...
public abstract class TransducerEvaluator
{
   ...
   */
	public void evaluate (TransducerTrainer tt,HashMap<String,Boolean> map,boolean flag)	{
		if (!precondition(tt))
			return;
		this.preamble(tt);
		for (int k = 0; k < instanceLists.length; k++)
			if (instanceLists[k] != null)
				evaluateInstanceList (tt, instanceLists[k], instanceListDescriptions[k],map,true);
	}
  ...
  public abstract void evaluateInstanceList (TransducerTrainer transducer, InstanceList instances, String description, HashMap<String,Boolean> map, boolean flag);
